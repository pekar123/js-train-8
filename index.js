...
function combineAndReverseArrays(arr1, arr2) {
  ...
  const element = arr1.concat(arr2);
  let revers = element.reverse();
  return element,revers
...
function findElementIndexes(arr, element) {
  ...
  const numfirst = arr.indexOf(element);
  const numlast = arr.lastIndexOf(element);
...
function customAt(arr, index) {
  if (!Array.isArray(arr)) {
    console.log('Невірний тип вхідного параметра. Повертаємо undefined.');
    return undefined;
  }

  if (index < 0 || index >= arr.length) {
    console.log('Індекс виходить за межі масиву. Повертаємо undefined.');
    return undefined;
  }
  ...
  if (typeof element === 'string') {
    ...
    console.log('Невідомий тип елементу');
  }
  return arr.at(index)
...
function customIncludes(arr, element) {
  ...
  const i = arr.filter((el) => el ===element).length
  return `Виведемо ${i} ${result}`
